...
import { useEffect, useState } from "react";
 ...
 export const DeleteServer = () => {
    ...
    console.log("delete serverId ", server?.id)
    ...
    <Dialog open={isModalOpen} onOpenChange={handleClose}>
        <DialogContent className="bg-white text-black overflow-auto">
            <DialogHeader className={"pt-5 px-6 text-center"}>
                <DialogTitle className="font-bold text-2xl text-center flex gap-x-3">Delete <p className="text-red-500 font-semibold">{server?.name}</p> Server?</DialogTitle>
